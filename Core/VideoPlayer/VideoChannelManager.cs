...
public class VideoChannelManager : ModSystem
{
    ...
    public static void AddPresetChannel(int channelId, ChannelContent content)
    {
        ...
        {
            ModContent.GetInstance<TerraVision>().Logger.Error($"Invalid preset channel ID: {channelId}. Must be between {MIN_CHANNEL} and {MAX_PRESET_CHANNEL}");
            return;
        }
        ...
        ModContent.GetInstance<TerraVision>().Logger.Info($"Added preset channel {channelId}");
    }
    ...
    public VideoPlayerCore GetOrCreateChannelPlayer(int channelId)
    {
        if (channelId < MIN_CHANNEL || channelId > MAX_CHANNEL)
        {
            TerraVision.instance.Logger.Error($"Invalid channel ID: {channelId}");
            return null;
        }
        ...
        if (!_channelPlayers.TryGetValue(channelId, out var player))
        {
            ...
            TerraVision.instance.Logger.Info($"Created shared player for channel {channelId}");
        }
    ...
    public void StartChannel(int channelId)
    {
        ...
        {
            TerraVision.instance.Logger.Warn($"Preset channel {channelId} has no content");
            return;
        }
    ...
    private void PlaySpecialEvent(SpecialEventVideo specialEvent)
    {
        ...
        {
            TerraVision.instance.Logger.Error($"[PlaySpecialEvent] Override channel player is null!");
            return;
        }

        TerraVision.instance.Logger.Info($"[PlaySpecialEvent] Starting special event");
        TerraVision.instance.Logger.Info($"[PlaySpecialEvent] Video URL: {specialEvent.VideoUrl}");
        TerraVision.instance.Logger.Info($"[PlaySpecialEvent] Repeat mode: {specialEvent.RepeatBehavior}");
        TerraVision.instance.Logger.Info($"[PlaySpecialEvent] Player initialized: {_overrideChannelPlayer.IsInitialized}");
        ...
        {
            TerraVision.instance.Logger.Info($"[PlaySpecialEvent] Video loaded and ready, activating override");
            ...
            Main.QueueMainThreadAction(() =>
            {
                ...
                TerraVision.instance.Logger.Info($"[PlaySpecialEvent] Paused {pausedCount} regular channels");

                _isOverrideChannelActive = true;
                _currentSpecialEvent = specialEvent;
                Main.NewText("⚠ SPECIAL BROADCAST INTERRUPTION ⚠", Color.Yellow);
                TerraVision.instance.Logger.Info($"[PlaySpecialEvent] Override channel activated");
            });
        };

        errorHandler = (sender, e) =>
        {
            TerraVision.instance.Logger.Error($"[PlaySpecialEvent] Video failed to load, canceling special event");
    ...
    private void OnSpecialChannelEnded(object sender, EventArgs e)
    {
        ...
        TerraVision.instance.Logger.Info("Special event video ended, returning to regular channels");
        ...
        TerraVision.instance.Logger.Info($"[OnSpecialChannelEnded] Resumed {resumedCount} regular channels");
    ...
    public void StopSpecialChannel()
    {
        ...
        _isOverrideChannelActive = false;
        _currentSpecialEvent = null;
        TerraVision.instance.Logger.Info("Special channel manually stopped");
    }
    ...
    private void PlayChannelContent(int channelId, ChannelContent channelContent, VideoPlayerCore player)
    {
        ...
        {
            TerraVision.instance.Logger.Warn($"Channel {channelId} has no entries");
            return;
        }
        ...
        Task.Run(async () =>
        {
            try
            {
                ...
                Main.QueueMainThreadAction(() =>
                {
                    ...
                    TerraVision.instance.Logger.Info($"Loaded playlist with {videoUrls.Count} videos (starting at #{startIndex + 1})");
                });
            }
            catch (Exception ex)
            {
    ...
    public void StopChannelIfUnused(int channelId)
    {
        ...
        {
            TerraVision.instance.Logger.Debug($"Channel {channelId} still has {activeTVCount} active TV(s)");
        }
    }
    ...
    public override void OnWorldLoad()
    {
        ...
        _overrideChannelPlayer.PlaybackReady += (s, e) => TerraVision.instance.Logger.Info("[OverrideChannel] PlaybackReady fired");
        _overrideChannelPlayer.PlaybackError += (s, e) => TerraVision.instance.Logger.Error("[OverrideChannel] PlaybackError fired");

        TerraVision.instance.Logger.Info($"Created override channel player (session: {_overrideChannelPlayer.SessionId})");

        TerraVision.instance.Logger.Info("VideoChannelManager: World loaded");
    }
    ...
    public override void OnWorldUnload()
    {
        ...
        TerraVision.instance.Logger.Info("VideoChannelManager: All channels disposed");
    }
    ...
    public override object Call(params object[] args)
    {
        try
        {
            ...
            switch (command)
            {
                ...
                default:
                    TerraVision.instance.Logger.Warn($"Unknown mod call command: {command}");
                    return false;
            }
        }
        catch (Exception ex)
        {
            TerraVision.instance.Logger.Error($"Mod call failed: {ex.Message}");
            return false;
        }
    }
    */
