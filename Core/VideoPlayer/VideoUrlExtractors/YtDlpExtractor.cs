...
public class YtDlpExtractor : IVideoUrlExtractor
{
    ...
    public async Task<string> GetDirectUrlAsync(string url, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            {
                TerraVision.instance.Logger.Warn($"yt-dlp failed to extract URL: {string.Join(", ", result.ErrorOutput ?? new string[0])}");
                return null;
            }
            ...
            {
                TerraVision.instance.Logger.Warn("yt-dlp returned empty URL");
                return null;
            }

            TerraVision.instance.Logger.Info($"yt-dlp extracted URL successfully");
            return directUrl;
        }
