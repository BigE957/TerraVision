...
public class ExampleVideoPlayerUI : UIState
{
    ...
    public override void Update(GameTime gameTime)
    {
        ...
        {
            if ((Main.mouseLeft && !_urlInput.IsMouseHovering) || (Main.keyState.IsKeyDown(Keys.Escape) && !Main.oldKeyState.IsKeyDown(Keys.Escape)))
            {
                _urlInput.Deselect();
                urlDeactivated = true;
            }
...
public class ExampleVideoUISystem : ModSystem
{
    ...
    public override void ModifyInterfaceLayers(System.Collections.Generic.List<GameInterfaceLayer> layers)
    {
        ...
        if (mouseTextIndex != -1)
        {
            layers.Insert(mouseTextIndex, new LegacyGameInterfaceLayer("TerraVision: Video Player UI", delegate
            {
                if (_videoUserInterface?.CurrentState != null)
